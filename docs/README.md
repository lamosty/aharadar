...
11. `local-codex.md` — optional personal Codex subscription mode.
12. `personal-server-systemd.md` — legacy/manual user-systemd split.
13. `adr/*` — decisions and tradeoffs; where we lock choices.
14. `recaps/*` — session recaps / audit log for AI-assisted development (handoff notes).
15. `learnings/*` - research notes / blog-draft material (informational).
