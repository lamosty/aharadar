...
function summarizeOutputItem(item: Record<string, unknown>, index: number): void {
  ...
  if (type.includes("tool") || toolName === "x_search") {
    console.log(`output[${index}] raw (truncated):`);
    console.log(truncateJson(item, 2000));
  }
}
...
async function main(): Promise<void> {
  ...
  const body = {
    ...
    input: [
      {
        role: "system",
        content: "Use the x_search tool to fetch posts. Then respond with the single word: ok.",
      },
      {
        role: "user",
