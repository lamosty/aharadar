  ...
  return {
    async getLatestByUser(userId: string): Promise<DigestRow | null> {
      const res = await db.query<DigestRow>(
        `select id, user_id, window_start::text as window_start, window_end::text as window_end, mode, created_at::text as created_at
         from digests
         where user_id = $1
    ...
    async upsert(params: {
      ...
      const res = await db.query<{ id: string; inserted: boolean }>(
        `insert into digests (user_id, window_start, window_end, mode)
         values ($1, $2::timestamptz, $3::timestamptz, $4)
         on conflict (user_id, window_start, window_end, mode)
         do update set window_start = excluded.window_start
         returning id, (xmax = 0) as inserted`,
        [params.userId, params.windowStart, params.windowEnd, params.mode]
      );
