      ...
      if (source.type === "x_posts") {
        ...
        if (parseErrors.length > 0) {
          ...
          const parseDetails = parseErrors.slice(0, 3).map((c) => ({
            ...
            emitResultsPresent: c.meta?.emit_results_present,
            emitResultsParseError: c.meta?.emit_results_parse_error,
            emitResultsArgsHead: c.meta?.emit_results_args_head,
            emitResultsArgsTail: c.meta?.emit_results_args_tail,
            emitResultsArgsLength: c.meta?.emit_results_args_length,
