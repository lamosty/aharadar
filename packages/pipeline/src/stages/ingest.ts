      ...
      if (source.type === "x_posts") {
        ...
        if (parseErrors.length > 0) {
          ...
          const parseDetails = parseErrors.slice(0, 3).map((c) => ({
            ...
            linesTotal: c.meta?.lines_total,
            linesValid: c.meta?.lines_valid,
            linesInvalid: c.meta?.lines_invalid,
            missingTimestampCount: c.meta?.missing_timestamp_count,
            missingHandleCount: c.meta?.missing_handle_count,
